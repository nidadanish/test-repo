...
class PaynaPayment implements IRedirectionPayment
{
    ...
    {
        if (!empty($this->order_info['payment_info']['msisdn']) && !empty($this->order_info['payment_info']['channel'])){
            ...
            $this->preparation_result->setData(
                $this->details_builder
                    ...
                    ->setPaymentUrl(fn_url("ec_payna_payment.redirect&order_id=".$this->order_info['order_id'], 'C'))
                    ->setReturnUrl(fn_url("ec_payna_payment.success&order_id=".$this->order_info['order_id'], 'C'))
                    ->setCancelUrl(fn_url("ec_payna_payment.failed&order_id=".$this->order_info['order_id'], 'C'))
                    ->asArray()
            );
        }else{
