...
class Manager
{
    ...
    protected $countries_cache;
     ...
     /** @var array $cities_cache */
     protected $cities_cache;

     /** @var string[] $districts_cache */
     protected $districts_cache;
    ...
    /** @var \Tygh\Addons\EcCustomApi\ProfileFields\Validator $validator */
    protected $validator;

    /** @var \Tygh\Addons\EcCustomApi\ProfileFields\Hydrator $hydrator */
    protected $hydrator;
    ...
    protected function getFieldValues(array $field, $lang_code)
    {
        switch ($field['field_type']) {
            ...
            case EC_CITY:
                return $this->getCities($lang_code);
            case EC_DISTRICT:
                return $this->getDistricts($lang_code);
     ...
     * @return array
     */
    protected function getCities($lang_code)
    {
        if ($this->cities_cache === null) {
            $_cities = fn_get_cities_tree($lang_code);
            $cities = array();
            foreach($_cities as $country=>&$states){
                foreach($states as $state=>&$_city){
                    foreach($_city as $city){
                        $cities[$country][$state][$city['city_id']] = $city['code'];
                    }
                }
            }
     ...
     * @return array
     */
    protected function getDistricts($lang_code)
    {
        if ($this->districts_cache === null) {
            $_districts= array();
            $districts = fn_get_districts_tree($lang_code);
            foreach($districts as $country=>&$states){
                foreach($states as $state=>&$cities){
                    foreach($cities as $city=>$district){
                        foreach($district as $_district){
                            $_districts[$country][$state][$city][$_district['district_id']] = $_district['code'];
                        }
                    }
                }
            }
            $this->districts_cache = $_districts;
        }
    ...
    /**
     * @see \Tygh\Addons\EcCustomApi\ProfileFields\Validator::validate
     *
     * @param array $schema
     * @param array $data
     *
    ...
    /**
     * @see \Tygh\Addons\EcCustomApi\ProfileFields\Hydrator::hydrate
     *
     * @param array $schema
     * @param array $data
     *
