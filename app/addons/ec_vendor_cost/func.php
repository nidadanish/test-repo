...
function fn_ec_vendor_cost_get_custom_price_data($product_data, $auth){
    if (AREA == 'C' || defined("ORDER_MANAGEMENT") || Registry::get('runtime.mode') == 'get_products_list' || Registry::get('runtime.mode') == 'cart_list' ||  Registry::get('runtime.mode') == 'build_queue' || (defined('API') && API)){
        ...
        if (!empty($product_data['ec_retail_price']) && defined('API') && $product_data['list_price'] < $product_data['price']){
            $product_data['list_price'] = $product_data['price'];
