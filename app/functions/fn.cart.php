...
function fn_get_set_taxes($taxes_set)
{
    ...
    sort($taxes_set);
    $key = base64_encode(json_encode($taxes_set));

    if (empty($mccd__taxes[$key])) {
        $mccd__taxes[$key] = db_get_hash_array("SELECT tax_id, address_type, priority, price_includes_tax, regnumber FROM ?:taxes WHERE tax_id IN (?n) AND status = 'A' ORDER BY priority", 'tax_id', $taxes_set);
    }

    return $mccd__taxes[$key];
}
...
function fn_calculate_tax_rates($taxes, $price, $amount, $auth, &$cart)
{
    ...
    foreach ($taxes as $key => $tax) {
        ...
        if (!empty($cart['stored_taxes']) && $cart['stored_taxes'] == 'Y' && (!empty($tax['rate_type']) || isset($cart['taxes'][$tax['tax_id']]['rate_value']))) {
            ...
            static $mccd__rates = [];
            $mccd__key = $tax['tax_id'] . '-' . $destination_id[$tax['address_type']];

            if (!isset($mccd__rates[$mccd__key])) {
                $mccd__rates[$mccd__key] = db_get_row("SELECT destination_id, rate_value, rate_type FROM ?:tax_rates WHERE tax_id = ?i AND destination_id = ?i", $tax['tax_id'], $destination_id[$tax['address_type']]);
            }

            $rate = $mccd__rates[$mccd__key];
