...
function fn_get_country_states($country_code, $avail_only = true, $lang_code = CART_LANGUAGE)
{
    ...
    static $mccd__states = [];
    $mccd__key = base64_encode($lang_code . '-' . $condition);

    if (!isset($mccd__states[$mccd__key])) {
        $mccd__states[$mccd__key] = db_get_hash_single_array("SELECT a.code, b.state FROM ?:states as a LEFT JOIN ?:state_descriptions as b ON b.state_id = a.state_id AND b.lang_code = ?s WHERE ?p ORDER BY b.state", array('code', 'state'), $lang_code, $condition);
    }

    return $mccd__states[$mccd__key];
}
