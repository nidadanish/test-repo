...
function fn_get_categories($params = array(), $lang_code = CART_LANGUAGE)
{
    ...
    if (AREA == 'C') {
        static $mccd__categories = [];
        $key = base64_encode($query_string);

        if (!isset($mccd__categories[$key])) {
            $mccd__categories[$key] = db_get_hash_array($query_string, 'category_id');
        }

        $categories = $mccd__categories[$key];

    } else {
        $categories = db_get_hash_array($query_string, 'category_id');
    }
