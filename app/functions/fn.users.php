...
function fn_get_profile_fields($location = 'C', $_auth = array(), $lang_code = CART_LANGUAGE, $params = array())
{
    ...
    static $mccd__fields = [];
    $mccd__key = base64_encode($select . '-' . $condition . '-' . $lang_code);

    if (!isset($mccd__fields[$mccd__key])) {
        $mccd__fields[$mccd__key] = db_get_hash_multi_array("SELECT ?:profile_fields.*, ?:profile_field_descriptions.description $select FROM ?:profile_fields LEFT JOIN ?:profile_field_descriptions ON ?:profile_field_descriptions.object_id = ?:profile_fields.field_id AND ?:profile_field_descriptions.object_type = 'F' AND lang_code = ?s $condition ORDER BY ?:profile_fields.position", array('section', 'field_id'), $lang_code);
    }

    $profile_fields = $mccd__fields[$mccd__key];
    $matches = array();
