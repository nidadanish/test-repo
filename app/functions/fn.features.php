...
function fn_get_product_features_list(array $product, $display_on = ProductFeaturesDisplayOn::CATALOG, $lang_code = CART_LANGUAGE)
{
    ...
    if (!empty($_data)) {
        ...
        if (!empty($_variant_ids)) {
            ...
            $mccd__key = base64_encode(json_encode($_variant_ids));

            if (!isset($mccd__images[$mccd__key])) {
                $mccd__images[$mccd__key] = fn_get_image_pairs($_variant_ids, 'feature_variant', 'V', true, true, $lang_code);
            }

            $images = $mccd__images[$mccd__key];
