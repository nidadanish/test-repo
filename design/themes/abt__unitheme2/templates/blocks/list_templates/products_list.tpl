{if $products}
    ...
    {foreach from=$products item=product key=key name="products"}
        ...
        {hook name="products:product_block"}
            ...
            {include file="common/product_data.tpl" product=$product min_qty=true product_labels_position="left-top"}

            <div class="ty-product-list {if $settings.abt__ut2.product_list.decolorate_out_of_stock_products == "YesNo::YES"|enum && $product.amount <= 0}decolorize{/if} clearfix"{if $ut2_load_more && $smarty.foreach.products.first} data-ut2-load-more="first-item"{/if}>
