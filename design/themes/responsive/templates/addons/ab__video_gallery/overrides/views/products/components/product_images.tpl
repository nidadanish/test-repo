...
{if $total_count == 0 || $total_count == 1}{$v_gal_width = 0}{elseif $total_count >= 6 && $settings.Appearance.thumbnails_gallery == "N"}{$v_gal_width = ($th_size * 2 + 18)}{else}{$v_gal_width = ($th_size + 12)}{/if}
...
{if $product.image_pairs || $ab__vg_videos}
    ...
    {if $settings.Appearance.thumbnails_gallery == "Y"}
                            ...
                            {if $ab__vg_videos && $addons.ab__video_gallery.position == 'pre'}
                                {foreach $ab__vg_videos as $video}
                                    ...
                                    <div class="cm-item-gallery">
                                        <a href="javascript:void(0)" data-ca-gallery-large-id="det_img_link_{$preview_id}_{$video.video_id}" class="ab__vg-image_gallery_item cm-gallery-item cm-thumbnails-mini ty-product-thumbnails__item gallery {if $ab__vg_settings.replace_image == 'Y' && $video@iteration == 1} active{/if}" style="width:{$th_size}px;height:{$th_size}px;" data-ca-image-order="{$image_counter}" data-ca-parent="#product_images_{$preview_id}">
        ...
        {/strip}
    {else}
        <div class="ab__vg-product-thumbnails ty-center{if $is_vertical} ab-vg-vertical-thumbnails" style="width: {$v_gal_width-4}px{/if}" id="images_preview_{$preview_id}">
            {strip}
                {if $ab__vg_videos && $addons.ab__video_gallery.position|default:"pre" == "pre"}
                    {foreach $ab__vg_videos as $video}
                        {$image_counter = $image_counter + 1}
                        <a href="javascript:void(0)" data-ca-gallery-large-id="det_img_link_{$preview_id}_{$video.video_id}" class="ab__vg-image_gallery_item ty-product-thumbnails__item cm-thumbnails-mini{if $ab__vg_settings.replace_image == 'Y' && $video@iteration == 1} active{/if}" style="width:{$th_size}px;height:{$th_size}px;" data-ca-image-order="{$image_counter}" data-ca-parent="#product_images_{$preview_id}">
                            {include file="addons/ab__video_gallery/components/video_icon.tpl" icon_width=$th_size icon_height=$th_size icon_alt=$video.title|strip_tags obj_id="`$preview_id`_`$video.video_id`_mini"}
                        </a>
                    {/foreach}
                {/if}
